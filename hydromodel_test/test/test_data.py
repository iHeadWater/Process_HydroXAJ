...
from hydromodel.test import definitions
...
def txt_file():
    return os.path.join(
        definitions.ROOT_DIR, "example", "01013500_lump_p_pe_q.txt"
    )


@pytest.fixture()
def json_file():
    return os.path.join(definitions.ROOT_DIR, "example", "data_info.json")


@pytest.fixture()
def npy_file():
    return os.path.join(
        definitions.ROOT_DIR, "example", "basins_lump_p_pe_q.npy"
    )
